...
import { Plus, Loader2, Tags, ExternalLink } from "lucide-react";
...
import { quickCreateBrand } from "../quick-create-brand";

type Props = {
  defaultName?: string;
  onCreated: (brand: { id: number; ad: string }) => void;
  variant?: "icon" | "button";
};

export function QuickCreateBrandDialog({ defaultName, onCreated, variant = "icon" }: Props) {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const r = await quickCreateBrand({ ad: ad.trim(), qeyd: qeyd.trim() });
      if (r.ok) {
        toast.success(`Brend yaradıldı: ${r.brand.ad}`);
        onCreated(r.brand);
        setOpen(false);
        setAd(""); setQeyd("");
      } else {
      ...
      <DialogTrigger asChild>
        {variant === "icon" ? (
          ...
          <Button size="sm" variant="outline">
            <Plus className="h-3.5 w-3.5" /> Yeni brend
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="md:max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-base">
            <Tags className="h-4 w-4" /> Sürətli brend yarat
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <div>
            ...
            <Input
              ...
              autoFocus required minLength={2} maxLength={120}
          ...
          <DialogFooter className="flex items-center justify-between gap-2 sm:justify-between">
            <Link
              href={`/anbar/markalar?yeni=1${ad ? `&name=${encodeURIComponent(ad)}` : ""}`}
