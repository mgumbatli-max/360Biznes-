...
export async function rotateRefreshToken(raw: string): Promise<{ sahibkarId: string; istifadeciId: string; cihaz: string | null; newRaw: string } | null> {
  // TOCTOU yarışını önləmək üçün findUnique+update+create eyni tranzaksiyada atomik icra olunur
  return prismaUnscoped.$transaction(async (tx) => {
    const row = await tx.mobil_refresh_tokens.findUnique({ where: { token_hash: hash(raw) } });
    if (!row || row.revoked_at || row.expires_at < new Date()) return null;
    await tx.mobil_refresh_tokens.update({ where: { id: row.id }, data: { revoked_at: new Date() } });
    const newRaw = randomBytes(48).toString("base64url");
    await tx.mobil_refresh_tokens.create({
      data: {
        sahibkar_id: row.sahibkar_id,
        istifadeci_id: row.istifadeci_id,
        token_hash: hash(newRaw),
        // cihaz sütunu VarChar(120) — real user-agent çox uzun olur, kəsirik (login 500-ün qarşısı)
        cihaz: row.cihaz ? String(row.cihaz).slice(0, 120) : null,
        expires_at: new Date(Date.now() + REFRESH_TTL_DAYS * 86_400_000),
      },
    });
