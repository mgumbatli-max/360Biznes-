...
/** Order in which result groups are rendered. */
...
function CommandPalette({
  ...
  // Refresh recent list whenever the dialog opens.
  useEffect(() => {
    if (open) {
  ...
  // Debounced server fetch for the actual query.
  useEffect(() => {
    ...
    // Secret-code shortcut: typing 4-8 digit number triggers sahibkar unlock check.
    // Default code is 7733 — owner can change in /sahibkar/ayarlar.
    // On match the server establishes a PIN session, so the user is redirected
    // directly into /sahibkar without entering PIN again.
  ...
  // Visible modules — pre-filtered by permission so navigation stays consistent.
  // Computed via useMemo so hooks order stays stable even when user is briefly null.
  ...
  // For typing matches on modules we filter by query.
      ...
      <DialogContent className="overflow-hidden p-0 md:max-w-2xl">
        ...
        <Command shouldFilter={false} className="bg-popover">
          ...
          <CommandList>
            ...
            {/* Modules section is always shown — also matched by query. */}
