...
export default async function StokPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  const [rows, anbarlar, products, kpis, recent] = await Promise.all([
    getStokRows(filter),
    getAnbarOptions(),
    getProductOptions(),
    getStokKpis(),
  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      <div className="space-y-4">
        ...
        {/* Hero — Stok dəyəri (col-span-2) + 3 statistika kartı = 5 sütun */}
        <section className="grid grid-cols-2 gap-3 lg:grid-cols-5">
          <div className="col-span-2 rounded-xl border border-emerald-500/25 bg-gradient-to-br from-emerald-500/10 to-transparent p-4">
            ...
            <div
              className="mt-2 text-3xl font-bold tabular-nums tracking-tight text-emerald-600 dark:text-emerald-400"
              title={formatMoney(kpis.deyer)}
            >
          ...
          <Stat
            label="Az stok"
            sub={kpis.az > 0 ? "kritik səviyyədə" : "hamı qaydadır"}
            value={formatCompactNumber(kpis.az)}
            icon={AlertTriangle}
            tone={kpis.az > 0 ? "warning" : "neutral"}
            fullValue={String(kpis.az)}
          />
          <Stat
            label="Stok yoxdur"
            sub={kpis.yox > 0 ? "tükənmiş" : "hamı stokda"}
            value={formatCompactNumber(kpis.yox)}
            icon={XCircle}
            tone={kpis.yox > 0 ? "danger" : "neutral"}
            fullValue={String(kpis.yox)}
          />
          <Stat
            label="Sağlamlıq %"
            sub="normal stok payı"
            value={`${kpis.saglamlik.toFixed(0)}%`}
            icon={Activity}
            tone={kpis.saglamlik >= 80 ? "success" : kpis.saglamlik >= 50 ? "warning" : "danger"}
          />
        </section>
