...
export default async function ContactDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [c, sales, purchases, stats, notes, comms, followups, finOps, tagLinks, managers, debtTimeline, servis, openSales, quickRefs, kpis360, health, journey, salesStats, documents] = await Promise.all([
    getContactDetail(id),
    getContactSalesHistory(id, 50),
    getContactPurchaseHistory(id, 50),
    getContactStats(id),
    getContactNotes(id),
    ...
    getServisHistoryForCustomer(id),
    getOpenSalesForCustomer(id, 50),
    getQuickRefs(),
    getCustomer360Kpis(id),
    getCustomerHealthScore(id),
    getCustomerJourney(id, 200),
    getCustomerSalesStats(id),
    ...
    <div className="mx-auto max-w-7xl space-y-5">
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
              ...
              <SalesStatBox
                ...
                label="Son satış"
                value={
                  salesStats.last_sale_at ? formatDate(salesStats.last_sale_at) : "—"
                }
                subline={
      ...
      <Tabs defaultValue="umumi">
        <TabsList className="w-full overflow-x-auto">
          ...
          {isCustomer && <TabsTrigger value="servis">Servis ({servis.stats.total})</TabsTrigger>}
          <TabsTrigger value="maliyye">Borc / Ödəniş</TabsTrigger>
          {isCustomer && <TabsTrigger value="sefer">Səfər ({journey.length})</TabsTrigger>}
          <TabsTrigger value="qeyd">Qeyd ({notes.length})</TabsTrigger>
          <TabsTrigger value="followup">Follow-up ({followups.length})</TabsTrigger>
          <TabsTrigger value="elaqe">Əlaqə tarixçəsi ({comms.length})</TabsTrigger>
          <TabsTrigger value="tapshiriqlar">Tapşırıqlar</TabsTrigger>
          <TabsTrigger value="sened">
            <FileText className="h-3 w-3" /> Sənəd ({documents.length})
          </TabsTrigger>
          <TabsTrigger value="tag">Tag ({tags.length})</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="qeyd" className="pt-3">
          <NotesTab kontragentId={c.id} initialNotes={notes} />
        </TabsContent>
        ...
        <TabsContent value="sened" className="pt-3">
          <SenedTab kontragentId={c.id} initial={documents} />
        </TabsContent>
        ...
        <TabsContent value="tag" className="pt-3">
          <Card className="glass">
            ...
            <CardContent>
              <TagsManager kontragentId={c.id} tags={tags} />
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
