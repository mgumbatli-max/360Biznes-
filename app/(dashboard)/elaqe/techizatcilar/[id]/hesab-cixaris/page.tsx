...
import {
  Phone, Mail, FileText, ChevronLeft, ArrowUpFromLine, ArrowDownToLine,
  RotateCcw, MessageCircle, FileDown, ShoppingBag,
} from "lucide-react";
import { getSupplierStatement, type SupplierStatementRow } from "@/features/maliyye/supplier-statement";
import { formatMoney, formatDate } from "@/lib/utils";
import { PrintButton } from "../../../musteriler/[id]/hesab-cixaris/print-button";

export const metadata: Metadata = { title: "Təchizatçı hesab çıxarışı" };

const NOV_META: Record<SupplierStatementRow["nov"], { label: string; icon: typeof FileText; tone: string }> = {
  alish: { label: "Alış qaiməsi", icon: ShoppingBag, tone: "border-amber-500/40 bg-amber-500/10 text-amber-700" },
  odenis: { label: "Ödəniş", icon: ArrowUpFromLine, tone: "border-emerald-500/40 bg-emerald-500/10 text-emerald-700" },
  qaytarma: { label: "Qaytarma", icon: RotateCcw, tone: "border-sky-500/40 bg-sky-500/10 text-sky-700" },
};
...
export default async function SupplierStatementPage({
  ...
  const waText = encodeURIComponent(
    `Salam, ${data.techizatci_ad}!\nHesab çıxarışınız:\nBaşlanğıc qalıq: ${formatMoney(data.baslangic_qaliq)}\nDövr borc: ${formatMoney(data.cemi_kredit)}\nDövr ödəniş: ${formatMoney(data.cemi_debet)}\nSon qalıq: ${formatMoney(data.son_qaliq)}`,
  );
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5 print:max-w-none">
      <div className="flex items-center justify-between print:hidden">
        <Link
          href={`/elaqe/techizatcilar/${id}`}
          className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground"
        >
          <ChevronLeft className="h-3.5 w-3.5" /> Təchizatçı kartına qayıt
        </Link>
        <div className="flex items-center gap-2">
          ...
          <a
            href={`/api/techizatci/${id}/hesab-cixaris.csv${sp.from || sp.to ? `?${new URLSearchParams(sp as Record<string, string>).toString()}` : ""}`}
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">{data.techizatci_ad}</h1>
          ...
          <div className="text-right">
            ...
            <div className="text-[10px] text-muted-foreground">
              {data.son_qaliq > 0 ? "Biz təchizatçıya borcluyuq" : data.son_qaliq < 0 ? "Təchizatçı bizə borcludur" : "Hesab qapalıdır"}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <Stat label="Dövr kredit (alış)" value={formatMoney(data.cemi_kredit)} tone="danger" />
        <Stat label="Dövr debet (ödəniş)" value={formatMoney(data.cemi_debet)} tone="success" />
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-border/60 bg-secondary/40 text-left text-[10.5px] uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th className="px-3 py-2.5 text-right">Debet (ödəniş)</th>
                <th className="px-3 py-2.5 text-right">Kredit (borc)</th>
                ...
                return (
                  <tr key={i} className="border-b border-border/30 hover:bg-secondary/30">
                    ...
                    <td className="px-3 py-2 font-mono text-[11px]">
                      ...
                      {r.qaime_nomresi && <div className="text-[10px] text-muted-foreground">Qaimə: {r.qaime_nomresi}</div>}
                    </td>
                    <td className="px-3 py-2 text-xs">{r.tesvir}</td>
                    <td className={`px-3 py-2 text-right tabular-nums ${r.debet > 0 ? "font-semibold text-emerald-600" : "text-muted-foreground"}`}>
                      {r.debet > 0 ? formatMoney(r.debet) : "—"}
                    </td>
                    <td className={`px-3 py-2 text-right tabular-nums ${r.kredit > 0 ? "font-semibold text-rose-600" : "text-muted-foreground"}`}>
            ...
            <tfoot className="border-t border-border/60 bg-secondary/40 text-xs">
              <tr>
                <td colSpan={4} className="px-3 py-2 font-semibold">Cəmi ({data.rows.length})</td>
                <td className="px-3 py-2 text-right font-bold tabular-nums text-emerald-600">{formatMoney(data.cemi_debet)}</td>
                <td className="px-3 py-2 text-right font-bold tabular-nums text-rose-600">{formatMoney(data.cemi_kredit)}</td>
